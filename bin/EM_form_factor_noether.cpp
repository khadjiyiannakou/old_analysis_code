...
void fit_GM(double* valueY, double* valueX, double* error, int Npoints, double fit_values[2]);
...
int main(int argc, char *argv[]){
  ...
  for(int iconf = 0 ; iconf < Nconfs ; iconf++){
    ...
    if(ptr_threep_electric == NULL || ptr_threep_magnetic == NULL || ptr_twop == NULL){
      fprintf(stderr,"Error: open CONF=%s\n",threep_Electric_names[iconf]);
      fprintf(stderr,"Error: open files for reading\n"); exit(EXIT_FAILURE);}
    // read threep
  ...
  ///////////////////////////////
    ...
    // fit to dipole to extract GM at zero //

    int fit_points=HIGHGM-LOWGM+1;
    int nan_points=0;
    for(int i = LOWGM; i <= HIGHGM; i++){ 
      if(i==7)nan_points++; // avoid nan_points
      if(i==15)nan_points++;
    }
    fit_points -= nan_points;
    double *valueY = (double*) calloc(fit_points,sizeof(double));
    double *valueX = (double*) calloc(fit_points,sizeof(double));
    double *errorY = (double*) calloc(fit_points,sizeof(double));
    double fit_values[2];
    double *GM0_bin = (double*) calloc(Nbins, sizeof(double));
    double *massDip_bin = (double*) calloc(Nbins, sizeof(double));

    int counter=0;
    for(int ibin = 0 ; ibin < Nbins ; ibin++){
      for(int i = LOWGM; i <= HIGHGM; i++){ 
	if(i==7)continue; // avoid nan_points
	if(i==15)continue;
	valueY[counter] = GM_b[ibin*MAXMOMSQ+i];
	valueX[counter] = q2GeV[i];
	errorY[counter] = dGM[i];
	counter++;
      }
      fit_GM(valueY,valueX,errorY,fit_points,fit_values);
      GM0_bin[ibin]=fit_values[0];
      massDip_bin[ibin]=fit_values[1];
      counter=0;
    }

    double GM0_bmean=0.;
    double massDip_bmean=0.;

    for(int ibin = 0 ; ibin < Nbins ; ibin++){
      GM0_bmean += GM0_bin[ibin];
      massDip_bmean += massDip_bin[ibin];
    }
    GM0_bmean /= Nbins;
    massDip_bmean /= Nbins;

    double temp_err_GM0=0.;
    double temp_err_massDip=0.;
    double err_GM0;
    double err_massDip;

    for(int ibin = 0 ; ibin < Nbins ; ibin++){
      temp_err_GM0 += (GM0_bmean-GM0_bin[ibin])*(GM0_bmean-GM0_bin[ibin]);
      temp_err_massDip += (massDip_bmean-massDip_bin[ibin])*(massDip_bmean-massDip_bin[ibin]);
    }

    err_GM0 = sqrt( (Nbins-1)/( (double) Nbins ) * temp_err_GM0  );
    err_massDip = sqrt( (Nbins-1)/( (double) Nbins ) * temp_err_massDip  );

    printf("GM(0) dipole fit: %+e %+e \t %+e %+e\n",GM0_bmean,err_GM0,massDip_bmean,err_massDip);
...
int dipoleFit(int m, int n, double *p, double *dy, double **dvec, void *vars)
{
  int i;
...
void fit_GM(double* valueY, double* valueX, double* error, int Npoints, double fit_values[2]){
  ...
  status = mpfit(dipoleFit, Npoints , 2, fit_values, 0, 0, (void *) &v, &result);
